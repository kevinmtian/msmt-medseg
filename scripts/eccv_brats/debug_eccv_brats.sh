...
is_conbr_loss=0
lambda_conbr=0.1
temperature_conbr=0.05
is_edge_only_conbr=0

is_con_late_loss=0
lambda_con_late=0.1
temperature_con_late=2.0
is_edge_only_con_late=0


# set generate parameter below
online_legacy_capacity=64
...
exp_folder="debug3-ft-rp${online_legacy_capacity}-nit${online_max_num_iterations}"
if [ $is_conbr_head == 1 ]
then
  echo "is_conbr_head"
...
EXP_ROOT="/data/<USERNAME>/checkpoints/eccv/eccv_nci/${exp_folder}"
