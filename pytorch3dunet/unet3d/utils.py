...
def save_checkpoint(state, is_best, checkpoint_dir, logger=None, suffix=None):
    ...
    if suffix is not None:
        last_file_path = os.path.join(checkpoint_dir, 'last_checkpoint_{}.pt'.format(suffix))
    else:
    ...
    if is_best:
        if suffix is not None:
            best_file_path = os.path.join(checkpoint_dir, 'best_checkpoint_{}.pt'.format(suffix))
        else:
            best_file_path = os.path.join(checkpoint_dir, 'best_checkpoint.pt')
        log_info(f"Saving best checkpoint to '{best_file_path}'")
        shutil.copyfile(last_file_path, best_file_path)
        return last_file_path, best_file_path
    return last_file_path, None
...
def get_logger(name, level=logging.INFO, filename=None):
    ...
    if loggers.get(name) is not None:
        ...
        # logging.basicConfig(filename=file_name, filemode='w', level=level)
...
class RunningAverage:
    ...
    def update(self, value, n=1):
        self.count += n
        self.sum += value * n
        self.avg = np.round(self.sum / self.count, 2)
...
def create_optimizer(optimizer_config, model):
    if optimizer_config is None:
        raise ValueError(f"we must specify an optimizer!")
    class_name = optimizer_config.get('name')
    # /data1/share/software/anaconda202011/lib/python3.8/site-packages/torch/optim/adam.py
    m = importlib.import_module('torch.optim')
    clazz = getattr(m, class_name)
    # add optimizer to the config
    optimizer_config_ = {k: v for k, v in optimizer_config.items() if k != "name"}
    return clazz(model.parameters(), **optimizer_config_)

def create_lr_scheduler(lr_config, optimizer):
    if lr_config is None:
        return None
    class_name = lr_config.get('name')
    m = importlib.import_module('torch.optim.lr_scheduler')
    clazz = getattr(m, class_name)
    # add optimizer to the config
    lr_config['optimizer'] = optimizer
    lr_config_ = {k: v for k, v in lr_config.items() if k != "name"}
    return clazz(**lr_config_)
