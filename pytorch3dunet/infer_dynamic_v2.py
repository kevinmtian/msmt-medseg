...
inference code corresponding to train_dynamic_v2.py
"""
...
def main():
    ...
    logger = get_logger("DynamicTrainingSetup", filename=args.log_dir.rstrip(".log") + "-infer.log")
    ...
    # create trainer
    print(f"MultiStageTrainer-Infer")
    ...
    # args.model_checkpoint is just the file name, not entire directory
    if not os.path.exists(args.model_checkpoint):
        raise ValueError(f"model {args.model_checkpoint} is invalid")
    # import pdb; pdb.set_trace()
    multistage_trainer.infer(
        model_checkpoint=args.model_checkpoint,
        save_for_paper_path=args.eccv_infer_save_dir,
    )
